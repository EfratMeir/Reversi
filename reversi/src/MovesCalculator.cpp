...
//	vector<Point> options_list;
	...
	for (int i = 0; i < board.getBoardSize(); i++) {
		for (int j = 0; j < board.getBoardSize(); j++) {
			Point p = board.all_players_list[i][j];
...
void MovesCalculator::checkRight(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign()
			...
			if (col - 1 >= 0 && board.all_players_list[row][col - 1].get_sign() == this_playr_sign) {
				//keep moving RIGHT as long as its the opponent sign || board borders
				while (col + i <= board.getBoardSize() && board.all_players_list[row][col + i].get_sign() == current_point.get_sign()) {
					i++;
				}
				...
				if (board.all_players_list[row][col + i].get_sign() == board.blank) {
					if (!allreadyInList(row, col + i)){
						options.push_back(board.all_players_list[row][col + i]);
					}
...
void MovesCalculator::checkLeft(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign() != board.blank) {
		...
		if (col + 1 <= board.getBoardSize() && board.all_players_list[row][col + 1].get_sign() == this_playr_sign) {
			//keep moving LEFT as long as its the opponent sign || board borders
			while (col - i >= 0 && board.all_players_list[row][col - i].get_sign() == current_point.get_sign()) {
				i++;
			}
			//if this point is outside the boards, return
			if (!(col - i >= 0 )){
...
void MovesCalculator::checkDown(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign() != board.blank) {
		...
		if (row - 1 >= 0 && board.all_players_list[row - 1][col].get_sign() == this_playr_sign) {
			//keep moving DOWN as long as its the opponent sign || board borders
			while (row + i <= board.getBoardSize() && board.all_players_list[row + i][col].get_sign() == current_point.get_sign()) {
				i++;
			}
			//if this point is outside the boards, return
			...
			if (board.all_players_list[row + i][col].get_sign() == board.blank) {
				if (!allreadyInList(row + i, col)){
					options.push_back(board.all_players_list[row + i][col]);
				}
			}
		}
	}
...
void MovesCalculator::checkUp(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign() != board.blank) {
		...
		if (row + 1 <= board.getBoardSize() && board.all_players_list[row + 1][col].get_sign() == this_playr_sign) {
			//keep moving UP as long as its the opponent sign || board borders
			while (row - i >= 0 && board.all_players_list[row - i][col].get_sign() == current_point.get_sign()) {
				i++;
			}
			//if this point is outside the boards, return
			if (!(row - i >= 0)){
...
void MovesCalculator::checkUpRightDiagonal(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign() != board.blank) {
		...
		if (row + 1 <= board.getBoardSize() && col - 1 >= 0 && board.all_players_list[row + 1][col - 1].get_sign() == this_playr_sign) {
			//keep moving UP-RIGHT as long as its the opponent sign || board borders
			while (row - i >= 0 && col + i <= board.getBoardSize() && board.all_players_list[row - i][col + i].get_sign() == current_point.get_sign()) {
				i++;
			}
			//if this point is outside the boards, return
			...
			if (board.all_players_list[row - i][col + i].get_sign() == board.blank) {
				if (!allreadyInList(row - i, col + i)){
					options.push_back(board.all_players_list[row - i][col + i]);
				}
...
void MovesCalculator::checkUpLeftDiagonal(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign() != board.blank) {
		...
		if (row + 1 <= board.getBoardSize() && col + 1 <= board.getBoardSize()
				&& board.all_players_list[row + 1][col + 1].get_sign() == this_playr_sign) {
			//keep moving UP-LEFT as long as its the opponent sign || board borders
			while (row - i >= 0 && col - i >= 0 && board.all_players_list[row - i][col - i].get_sign() == current_point.get_sign()) {
				i++;
			}
			//if this point is outside the boards, return
			if (!(row - i >= 0 && col - i >= 0)){
...
void MovesCalculator::checkDownRightDiagonal(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign() != board.blank) {
		...
		if (row - 1 >= 0 && col - 1 >= 0 && board.all_players_list[row - 1][col - 1].get_sign() == this_playr_sign) {
			//keep moving DOWN-RIGHT as long as its the opponent sign || board borders
			while (row + i <= board.getBoardSize() && col + i <= board.getBoardSize() && board.all_players_list[row + i][col + i].get_sign() == current_point.get_sign()) {
				i++;
			}
			//if this point is outside the boards, return
...
void MovesCalculator::checkDownLeftDiagonal(Board board, Point current_point,
	...
	if (current_point.get_sign() != this_playr_sign && current_point.get_sign() != board.blank) {
		...
		if (row - 1 >= 0 && col + 1 <= board.getBoardSize() && board.all_players_list[row - 1][col + 1].get_sign() == this_playr_sign) {
			//keep moving DOWN-LEFT as long as its the opponent sign || board borders
			while (row + i <= board.getBoardSize() && col - i >= 0 && board.all_players_list[row + i][col - i].get_sign() == current_point.get_sign()) {
				i++;
			}
			//if this point is outside the boards, return
